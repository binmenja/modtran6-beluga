function [] = modtran6_tape5_make_brb(path_tape5,nlyr,z,p,t,q,co2,o3,resolution,fwhm,cloud,v1,v2,angle,iLoc,emis,zt,ts,modroot)
    % This program generates tape5 file for the modtran computation
    % : structure; include: cloud.qi, ice particle density g/m^3
    ...
    %%% modroot.in
    % Set default value for modroot if not provided by the user
    if nargin < 19 || isempty(modroot)
        modroot = 'mlw';
    end
    ...
    %q = q .* 1.1;
    %q = q .* 1e-3;
    %tmp_data = Es_Magnus(t+1)./Es_Magnus(t).*(q./(q+0.622));
    %q = tmp_data./(1-tmp_data).*0.622;
    %q = q .* 1e3;
  ...
  if card2.icld >0
    ...
    %%% write to file
    filename = strcat(path_tape5, tape5_name);
    disp(filename);
