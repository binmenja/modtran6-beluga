function [] = modtran6_tape5_maker(path_modtran,nlyr,z,p,t,q,co2,o3,resolution,fwhm,cloud,v1,v2,angle,iLoc,emis,zt,ts)
    % This program generates tape5 file for the modtran computation
    % This script was initially written by J. Feng and modified by B. Riot Bretecher.
    % For further information, contact B. Riot Bretecher (benjamin.riotbretecher@mail.mcgill.ca)
    % As is, this script is designed to result in Line By Line outputs (0.1 cm-1 resolution) for user-specified profile.
    if nargin < 1
    path_modtran = '';
    end
    
    %%% modroot.in
    modroot = ['mlw']; % change this to your own root name
    fid = fopen('modroot.in','w');
    fprintf(fid,'%s\n',modroot);
    fclose(fid);
    
    tape5_name = [modroot, '.tp5'];
    zenith_angle = 180-angle; % If angle = 180, zenith view, if angle = 0, nadir view
    ...
    % v2: cm-1
    % angle: degrees
    % iLoc: 1-5 ; read the modtran manual for more information
    % emis: 0-1; boundary emissivity, usually 0 if DLR and ~0.8 if OLR
    % ts: K
    % zt: km
    ...
    %%% set the values
    % card 1
    card1.modtran = 'H';                 % T(M) / C(K) / F(L), H for LBL
    ...
    card1.t_best =' ';                   % T
    card1.itype = 2;                     % 1-3
    card1.iemsct =1;                    % 0-3, 0 transmittance, 1 radiance
    card1.imult = 0;                     % 0, +-1
    card1.m1 = 0;                        % T and q
    card1.m2 = 0;                        % H2O
    card1.m3 = 0;                        % O3
    card1.m4 = 0;                        % CH4
    card1.m5 = 5;                        % N2O ; adapt to your own need
    card1.m6 = 5;                        % CO ; adapt to your own need
    ...
    card1.tptemp = t(end);                    % boundary temperature, > 0 / <= 0
    end
    ...
    card1a.co2mx = 400;                  % 0(blank, 330), or another value
    card1a.h2ostr = 1;                 % 0(blank) / gxx / axx / positive factor
    card1a.o3str = 1;                    % 0(blank) / gxx / axx / positive factor
    card1a.c_prof = 0;                   % 0 - 7
    card1a.lsunfl = 'f' ;                  % T/F
    card1a.lbmnam = 4 ;                  % F/4/2 % changed to f for transmittance
    card1a.lfltnm = 'F' ;                  % T/F
    card1a.h2oaer = ' ';                 % 
    card1a.cdtdir = 'T';
    card1a.solcon = 0;
    ...
    % card 2
    card2.aplus = '  ';                  % blank / A+
    card2.ihaze = 0;                     % -1, 0, 1-10
    card2.cnovam = ' ';                  % blank / N
    card2.iseasn = 2;                    % 0-2 ; 2 is fall-winter
    ...
    card2a.calt = -9.0;
    card2a.cext = -9.0;
    card2a.ncralt = -9; %length(cloud.z);     %%% number of layers input
    card2a.ncrspc = -9;%223;                 %%% number of bands
    card2a.cwavln = -9.0;
    card2a.ccolwd = -9.0;
    card2a.ccolip = -9.0;
    card2a.chumid = 500;
    ...
    card2c.ird2 = 0;                     % 0-2
    card2c.hmodel = 'modtran6_tape5_make_brb.m';
    card2c.ree = ' ';
    card2c.nmolyc = 0;
    card2c.e_mass = 0;
    card2c.airmwt = 0;
  ...
  if card2.icld >0
      ...
      if  ~isfield(cloud,'habit')
          cloud.habit='solid_column_r050_hres';
      end
    ...
    card4.xflag = 'W';                   % W / M / N
    card4.dlimit = '        ';
    card4.flags = ' S     '; % S for sinc function instead of triangular
    card4.mlflx = '   ';
    card4.vrfrac = '          ';
    % card 5
